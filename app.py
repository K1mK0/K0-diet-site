...
# 2. 카테고리별 데이터 정의 (avoid: 피할 단어, random_pool: 없음 선택시 무작위 메뉴)
condition = {}
is_free_mode = False
...
if "선택 안 함" in category:
    ...
    condition = {
        ...
        "reason": "혈당 스파이크를 유발하는 정제 탄수화물을 배제하고, 식후 졸음을 방지하는 단백질·채소 중심 구성"
    }
    ...
    condition = {
        ...
        "reason": "포화지방과 단순당을 제한하고 포만감을 유지하는 저칼로리 고단백 식품 중심"
    }
    ...
    try:
        ...
        if is_free_mode:
            ...
            if is_random_mode:
                ...
                st.markdown(f"### 🔍 검색 결과: {condition['name']} ('{search_keyword}' 관련)")
                st.info(f"💡 **식품영양학적 추천 원리:** {condition['reason']}")
