...
# 웹페이지 기본 설정
st.set_page_config(page_title="수험생 맞춤형 푸드 코치", page_icon="🥗", layout="centered")

st.title("🥗 수험생 & 청소년 고민 해결 맞춤 식단 추천")
st.markdown("**[발표자: 20502 김가영 / 진로: 식품공학과]**")
st.markdown("공부하느라 지친 우리 또래들의 현실적인 식습관 고민을 해결해 주는 스마트 푸드 가이드입니다.")
st.markdown("---")

# 1. 고민 카테고리 선택
category = st.selectbox(
    "💡 해결하고 싶은 오늘의 고민을 선택해주세요:",
    (
...
if "식곤증" in category:
    condition = {
        ...
        "avoid": ['라면', '면', '짜장면', '피자', '햄버거', '빵', '과자', '설탕', '떡볶이'], # 혈당을 급격히 올리는 정제 탄수화물 회피
    ...
    # 대체제 모드는 사용자가 먹고 싶은 음식을 직접 입력받아 건강한 버전으로 변환
    ...
    if craving:
        condition = {
            ...
            "reason": f"학생들이 자주 찾는 고열량 간식인 '{craving}'의 영양적 문제점(높은 나트륨·당류)을 보완하고, 대체 식재료를 활용한 건강 지향적 레시피 매칭"
        }
    else:
        condition = {
            "name": "건강한 대체 식단",
...
# 만약 대체제 카테고리가 아니라면 재료 입력받기
if "대체제" not in category:
    user_input = st.text_input("🛒 집에 있거나 요리하고 싶은 주재료를 입력하세요 (예: 닭가슴살, 두부, 현미, 달걀):", "두부")
else:
...
if st.button("🔍 맞춤형 식단 추천받기"):
    if not user_input:
        st.warning("재료나 먹고 싶은 음식을 입력해주세요!")
    else:
